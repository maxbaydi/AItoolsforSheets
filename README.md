...
### `AIQ(promptTemplate, [dataRange], [temperatureValue])`

Отправляет простой текстовый запрос к AI.

*   **`promptTemplate`**: Строка вашего запроса. Может содержать плейсхолдер `{data}` для вставки данных из `dataRange`.
*   **`dataRange`** (необязательный): Ячейка или диапазон ячеек, данные из которых будут вставлены в `promptTemplate` вместо `{data}`.
*   **`temperatureValue`** (необязательный): Число, определяющее степень случайности ответа AI (например, `0.7`). Если не указано, используется значение из глобальных настроек.

**Возвращает**: Текстовый ответ от AI.

**Примеры использования:**

*Примечание: В примерах ниже используется запятая (`,`) в качестве разделителя аргументов. Если в ваших региональных настройках Google Таблиц используется точка с запятой (`;`) в качестве разделителя, пожалуйста, замените запятые на точки с запятой в формах.*

1.  **Простой запрос из ячейки:**
    Если в ячейке `A1` текст: "расскажи интересный факт о кошках"
    *   `=AIQ(A1)` или `=AIQ(A1)`
        *   Отправит запрос: "расскажи интересный факт о кошках"

2.  **Запрос с данными из другой ячейки:**
    Если в ячейке `B1` текст: "Эйфелева башня"
    *   `=AIQ("Напиши краткое описание для: {data}", B1)` или `=AIQ("Напиши краткое описание для: {data}"; B1)`
        *   Отправит запрос: "Напиши краткое описание для: Эйфелева башня"

3.  **Запрос с данными из диапазона ячеек:**
    Если в ячейках `C1` текст "Яблоко", `C2` текст "Апельсин", `C3` текст "Банан".
    *   `=AIQ("Составь список покупок из следующих фруктов: {data}", C1:C3)` или `=AIQ("Составь список покупок из следующих фруктов: {data}"; C1:C3)`
        *   `dataRange` (C1:C3) будет обработан как "Яблоко\nАпельсин\nБанан".
        *   Отправит запрос: "Составь список покупок из следующих фруктов: Яблоко\nАпельсин\nБанан"

4.  **Запрос с указанием температуры (степени креативности):**
    *   `=AIQ("Напиши короткий стих о весне", , 0.9)` или `=AIQ("Напиши короткий стих о весне"; ; 0.9)`
        *   Отправит запрос "Напиши короткий стих о весне" с температурой 0.9 (более креативный ответ).
        *   Обратите внимание на двойную запятую (или две точки с запятой), так как параметр `dataRange` пропущен.

5.  **Запрос без плейсхолдера {data}, но с dataRange (dataRange будет проигнорирован для вставки):**
    *   `=AIQ("Какой сегодня день?", A1)` или `=AIQ("Какой сегодня день?"; A1)`
        *   Отправит запрос: "Какой сегодня день?" (содержимое A1 не будет использовано в `{data}`, так как его нет в шаблоне).

Функция `AIQ` также включает базовые инструкции для модели: "Ответь только на поставленный вопрос или выполни инструкцию. Не добавляй никаких вступлений, объяснений, извинений или дополнительного текста, кроме самого ответа. Не используй Markdown или HTML разметку." Это помогает получать более чистые и прямые ответы. Используется кэширование для ускорения повторных одинаковых запросов.
